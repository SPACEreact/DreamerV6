...
export class DualProviderImageService {
  ...
  private async initializeProviders(): Promise<void> {
    try {
      ...
      if (!stableDiffusionConfig.apiKey) {
        ...
        showErrorToast('Missing Hugging Face token');
      ...
      if (!geminiConfig.apiKey) {
        ...
        showErrorToast('Missing Gemini API key');
        throw new Error(message);
      }
  ...
  async generateDualProvider(request: DualProviderRequest): Promise<DualProviderResponse> {
    ...
    try {
      ...
      if (resultA.status === 'rejected' && resultB.status === 'rejected') {
        showErrorToast('Both providers failed to generate image');
      ...
      if (resultA.status === 'fulfilled' && resultB.status === 'fulfilled') {
        ...
        showSuccessToast(`Image generated successfully by ${selectedProvider}`);
      } else {
  ...
  async generateWithFallback(
    ...
    try {
      ...
      showSuccessToast(`Image generated by ${primary}`);
      return response;
      ...
      try {
        ...
        showWarningToast(`Image generated by ${secondary} (fallback)`);
        return response;
