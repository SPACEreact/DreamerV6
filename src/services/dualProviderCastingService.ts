  ...
  async generateDualProvider(request: DualProviderCastingRequest): Promise<DualProviderCastingResponse> {
    ...
    try {
      ...
      if (!primaryResponse && !secondaryResponse) {
        ...
        showErrorToast({ 
          title: 'Both casting providers failed',
          description: 'Unable to generate casting recommendations'
        });
      ...
      if (!primaryResponse && secondaryResponse) {
        showWarningToast({ 
          title: `Provider ${request.providerA} failed, using ${request.providerB}`
        });
        primaryResponse = secondaryResponse;
        secondaryResponse = undefined;
      } else if (primaryResponse && !secondaryResponse) {
        showWarningToast({ 
          title: `Provider ${request.providerB} failed, using ${request.providerA}`
        });
      }
      ...
      showSuccessToast({ 
        title: 'Casting recommendations generated',
        description: `${consensusRecommendations.length} actors recommended`
      });
  ...
  async generateWithFallback(
    ...
    try {
      ...
      if (fallbackProviderId) {
        ...
        if (fallbackProvider) {
          showWarningToast({ 
            title: `Failing over to ${fallbackProviderId}`
          });
