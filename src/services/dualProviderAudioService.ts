...
export class DualProviderAudioService {
  ...
  async generateDualProvider(request: DualProviderAudioRequest): Promise<DualProviderAudioResponse> {
    ...
    try {
      ...
      if (resultA.status === 'rejected' && resultB.status === 'rejected') {
        showErrorToast('Both providers failed to generate audio');
      ...
      if (resultA.status === 'fulfilled' && resultB.status === 'fulfilled') {
        ...
        showSuccessToast(`Audio generated successfully by ${selectedProvider}`);
      } else {
        ...
        if (resultA.status === 'fulfilled') {
          ...
          showWarningToast(`Using ${selectedProvider} (fallback)`);
      ...
      showErrorToast('Audio generation failed');
  ...
  async generateWithFallback(
    ...
    try {
      ...
      showSuccessToast(`Audio generated by ${primary}`);
      return response;
      ...
      try {
        ...
        showWarningToast(`Audio generated by ${secondary} (fallback)`);
        return response;
  ...
  async batchGenerate(
    ...
    if (errors.length > 0) {
      showWarningToast(`${errors.length} of ${requests.length} audio generations failed`);
    } else {
      showSuccessToast(`All ${requests.length} audio files generated successfully`);
    }
